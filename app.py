...
# Tablo Yapılarını Güncelle (Stok takibi eklendi)
...
if choice == "📦 Ürün Yönetimi":
    ...
    st.dataframe(pd.read_sql_query("SELECT ad as 'Ürün', birim_fiyat as 'Fiyat', stok_miktari as 'Mevcut Stok' FROM urunler", conn))
...
# 3. SATIŞ PANELİ (STOKTAN DÜŞME VE PDF SİMÜLASYONU)
        ...
        with st.form("satis_form"):
            ...
            if submitted:
                ...
                if u_row['stok_miktari'] >= miktar:
                    ...
                    conn.commit()
                    st.success(f"Satış Başarılı! Toplam Tutar: {toplam} TL. Kalan Stok: {yeni_stok}")
                    
                    # BASİT PDF/TEXT ÇIKTI ÖNİZLEMESİ
                    fatura_icerik = f"SATIŞ FATURASI\nTarih: {tarih}\nMüşteri: {secilen_musteri}\nÜrün: {secilen_urun}\nMiktar: {miktar}\nToplam: {toplam} TL"
                    st.download_button("📄 Fatura/İrsaliye Dökümü Al (TXT)", fatura_icerik, file_name=f"fatura_{secilen_musteri}.txt")
                else:
                    st.error(f"Yetersiz Stok! Mevcut: {u_row['stok_miktari']}")
    else:
        st.warning("Lütfen önce ürün ve müşteri tanımlayın.")

# 4. DASHBOARD & PİVOT (TARİH FİLTRELİ)
